(function ($) {
    ...
    $.validate({
        ...
        onSuccess: function () {
            if ($("#ftitle").html().substr(0, 4) == "Edit") {
                //--- Edit
                $.post('master_data/m_customer_job/update', $("#mainForm").serialize() + "&id=" + $("body").data("id"), function (obj) {
                    if (obj.msg == 1) {
                        $("#mainForm")[0].reset();
                ...
                //--- Insert
                $.post('master_data/m_customer_job/insert', $("#mainForm").serialize(), function (obj) {
                    if (obj.msg == 1) {
                        $("#mainForm")[0].reset();
                        $('#mainTable table').DataTable().ajax.reload();
    ...
    $('#mainTable').on('click', 'a[title^=Edit]', function (e) {
        ...
        $("#ftitle").html('Edit');
        $("#customer_job_name").val(d.customer_job_name).focus();
    ...
    var t = $('#mainTable table').DataTable({
        ...
        ajax: {
            url: baseUrl + 'master_data/m_customer_job/getdata',
            type: 'POST'
        },
        columns: [
    ...
    $('#mainTable').selectDTBks(t, 'master_data/m_customer_job/delete');
